namespace TESVSnip.UI
{
    ...
    internal static class Spells
    {
        private static readonly string[] LooseGroups = new[]
                                                           {
                                                               "CELL", "WRLD", "REFR", "ACRE", "ACHR", "NAVM", "GRUP"//, "DIAL", 
                                                           };
                    ...
                    foreach (var srcRec in src.Where(x => LooseGroups.Contains(x.Name)))
                    {
                        ...
                        foreach (var baseRecord in dstnodes)
                        {
                            ...
                            if (baseRecord is GroupRecord)
                            {
                                ...
                                /*DHK for INFO GRUP*/
                                var pargr = par.Records.OfType<GroupRecord>().FirstOrDefault(x => x.ContentsType == GetGroupParentContentType(gr.GroupType));
                                if (pargr != null && pargr.ContentsType.Length == 4)
                                {
                                    par = pargr;
                                    //continue;
                                }
        ...
        //DHK
        public static string GetGroupParentContentType(uint groupType)
        {
            switch (groupType)
            {
                case 7:
