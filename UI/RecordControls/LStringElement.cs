namespace TESVSnip.UI.RecordControls
{
    ...
    internal partial class LStringElement : TextElement
    {
        ...
        private void SetTextByID()
        {
            ...
            if (s == null)
            {
                XDocument docStrings = XDocument.Load(@"d:\Work\C#\Snip\dev-nogardeht\!Work\XML\xml.xml");

                var dString = docStrings.Root.Descendants("String");

                foreach (var ss in dString)
                {
                    if (ss.Attribute("sID").Value.ToString() == id.ToString("X6"))
                    {
                        s = ss.Descendants("Source").ToList()[0].Value.ToString();
                        break;
                    }
                }
            }
